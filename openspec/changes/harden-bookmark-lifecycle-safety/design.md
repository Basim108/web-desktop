...
`bookmarkSettings`/`folderSettings`/`gridSettings` are canvas-layout state, so their cleanup is a `bookmark-canvas` requirement. The custom icon blob is owned by the icon feature (`bookmark-icons` already has a user-initiated "Custom Icon Removal" requirement), so removal-triggered icon cleanup is a new requirement in that same capability rather than duplicated in `bookmark-canvas`. This mirrors how the code itself is split: `events.ts`'s `onRemoved` handler calls into both `storage/` (settings) and `iconDb.ts` (icon blob) rather than one owning both.
